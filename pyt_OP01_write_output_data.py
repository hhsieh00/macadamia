...
def retrieve_output_data(output_data_filepath,sqlite_file,path_logfile,path_errorfile):
    try:
        ...
        with open(output_data_filepath,'w') as of:
            of.write('detection_id   desig_num   telescope                        instrument                   \
...
proc_data_file                          preview_img_file\n')
        with open(output_data_detections_filepath,'w') as of:
            of.write('detection_id   desig_num   telescope                        instrument                   \
tel_apsize   obs_code   image_ext   filter       obs_date     \
expstart_tai      expstart_jd   exptime   airmass    \
trackrate_ra   trackrate_dec    zpoint   zpoint_err   zpt_nstars   \
...
ascnode      meananm   raw_data_link                                                                                  \
proc_data_file                          preview_img_file\n')
        with open(output_data_nondetections_filepath,'w') as of:
            of.write('detection_id   desig_num   telescope                        instrument                   \
tel_apsize   obs_code   image_ext   filter       obs_date     \
expstart_tai      expstart_jd   exptime   airmass    \
trackrate_ra   trackrate_dec    zpoint   zpoint_err   zpt_nstars   \
...
WHERE ddat.exposure_id != -1"
        ...
        if rows != None:
            with open(output_data_filepath,'a') as of_1, open(output_data_detections_filepath,'a') as of_2, open(output_data_nondetections_filepath,'a') as of_3:
                for row in rows:
                    det_id            = int(row[0])
