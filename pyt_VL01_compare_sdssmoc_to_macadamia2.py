...
def match_detection(idx_sdss,sdss_filter,mag_sdss,merr_sdss,sdssmoc_table,detection_id_mac,desig_num_mac,exp_start_jd_mac,mag_mac,magerr_mac,path_logfile):
    #idx_mac = 0
    mac_entry_found = False
    idx_mac,mag_diff_sdss_mac,mag_diff_err_sdss_mac = 0,0,0
    if sdss_filter == 'g':
        for idx_mac in range(0,len(desig_num_mac)):
            if desig_num_mac[idx_mac] == sdssmoc_table['ast_num'][idx_sdss]:
                if abs((exp_start_jd_mac[idx_mac]-2400000.5)-sdssmoc_table['mjd'][idx_sdss]) < 0.005:
                    ...
                    if sdssmoc_table['gmag'][idx_sdss] != 99.99:
                        mag_diff_sdss_mac = mag_mac[idx_mac] - mag_sdss
                        mag_diff_err_sdss_mac = (magerr_mac[idx_mac]**2 + merr_sdss**2)**0.5
                    return mac_entry_found,idx_mac,mag_diff_sdss_mac,mag_diff_err_sdss_mac
    if sdss_filter == 'r':
        for idx_mac in range(0,len(desig_num_mac)):
            if desig_num_mac[idx_mac] == sdssmoc_table['ast_num'][idx_sdss]:
                if abs((exp_start_jd_mac[idx_mac]-2400000.5)-sdssmoc_table['mjd'][idx_sdss]) < 0.005:
                    mac_entry_found = True
                    mcd.output_log_entry(path_logfile,'>> SDSS entry {:d} ({:s}-band) matched to MACADAMIA detection {:d}.'.format(idx_sdss,sdss_filter,int(detection_id_mac[idx_mac])))
                    if sdssmoc_table['rmag'][idx_sdss] != 99.99:
                        mag_diff_sdss_mac = mag_mac[idx_mac] - mag_sdss
                        mag_diff_err_sdss_mac = (magerr_mac[idx_mac]**2 + merr_sdss**2)**0.5
                    return mac_entry_found,idx_mac,mag_diff_sdss_mac,mag_diff_err_sdss_mac
    if sdss_filter == 'i':
        for idx_mac in range(0,len(desig_num_mac)):
            if desig_num_mac[idx_mac] == sdssmoc_table['ast_num'][idx_sdss]:
                if abs((exp_start_jd_mac[idx_mac]-2400000.5)-sdssmoc_table['mjd'][idx_sdss]) < 0.005:
                    mac_entry_found = True
                    mcd.output_log_entry(path_logfile,'>> SDSS entry {:d} ({:s}-band) matched to MACADAMIA detection {:d}.'.format(idx_sdss,sdss_filter,int(detection_id_mac[idx_mac])))
                    if sdssmoc_table['imag'][idx_sdss] != 99.99:
                        mag_diff_sdss_mac = mag_mac[idx_mac] - mag_sdss
                        mag_diff_err_sdss_mac = (magerr_mac[idx_mac]**2 + merr_sdss**2)**0.5
                    return mac_entry_found,idx_mac,mag_diff_sdss_mac,mag_diff_err_sdss_mac
    if sdss_filter == 'z':
        for idx_mac in range(0,len(desig_num_mac)):
            if desig_num_mac[idx_mac] == sdssmoc_table['ast_num'][idx_sdss]:
                if abs((exp_start_jd_mac[idx_mac]-2400000.5)-sdssmoc_table['mjd'][idx_sdss]) < 0.005:
                    mac_entry_found = True
                    mcd.output_log_entry(path_logfile,'>> SDSS entry {:d} ({:s}-band) matched to MACADAMIA detection {:d}.'.format(idx_sdss,sdss_filter,int(detection_id_mac[idx_mac])))
                    if sdssmoc_table['zmag'][idx_sdss] != 99.99:
                        mag_diff_sdss_mac = mag_mac[idx_mac] - mag_sdss
                        mag_diff_err_sdss_mac = (magerr_mac[idx_mac]**2 + merr_sdss**2)**0.5
                    return mac_entry_found,idx_mac,mag_diff_sdss_mac,mag_diff_err_sdss_mac
    return mac_entry_found,idx_mac,mag_diff_sdss_mac,mag_diff_err_sdss_mac
...
def create_sdss_macadamia_comparison_files2(sdssmoc_table,macadamia_output_filepath,sdss_vs_macadamia_filepath,keep_going,path_logfile,path_errorfile):
    ...
    if keep_going:
        ...
        macadamia_output_g_filepath         = macadamia_output_filepath[:-4] + '_g.dat'
        macadamia_output_r_filepath         = macadamia_output_filepath[:-4] + '_r.dat'
        macadamia_output_i_filepath         = macadamia_output_filepath[:-4] + '_i.dat'
        macadamia_output_z_filepath         = macadamia_output_filepath[:-4] + '_z.dat'
        ...
        detection_id_mac_g    = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(0))
        detection_id_mac_r    = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(0))
        detection_id_mac_i    = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(0))
        detection_id_mac_z    = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(0))
        mcd.output_log_entry(path_logfile,'Reading in numerical designations...')
        desig_num_mac_g       = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(1))
        desig_num_mac_r       = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(1))
        desig_num_mac_i       = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(1))
        desig_num_mac_z       = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(1))
        mcd.output_log_entry(path_logfile,'Reading in exposure start times...')
        exp_start_jd_mac_g    = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(10))
        exp_start_jd_mac_r    = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(10))
        exp_start_jd_mac_i    = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(10))
        exp_start_jd_mac_z    = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(10))
        mcd.output_log_entry(path_logfile,'Reading in distances to predicted positions...')
        dist_pred_postn_mac_g = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(31))
        dist_pred_postn_mac_r = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(31))
        dist_pred_postn_mac_i = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(31))
        dist_pred_postn_mac_z = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(31))
        mcd.output_log_entry(path_logfile,'Reading in measured magnitudes and uncertainties...')
        mag_mac_g             = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(34))
        mag_mac_r             = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(34))
        mag_mac_i             = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(34))
        mag_mac_z             = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(34))
        magerr_mac_g          = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(35))
        magerr_mac_r          = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(35))
        magerr_mac_i          = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(35))
        magerr_mac_z          = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(35))
        mcd.output_log_entry(path_logfile,'Reading in SNRs...')
        snr_mac_g             = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(37))
        snr_mac_r             = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(37))
        snr_mac_i             = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(37))
        snr_mac_z             = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(37))
        mcd.output_log_entry(path_logfile,'Reading in trail lengths and FWHMs...')
        trail_length_mac_g    = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(38))
        trail_length_mac_r    = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(38))
        trail_length_mac_i    = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(38))
        trail_length_mac_z    = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(38))
        trail_fwhm_mac_g      = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(39))
        trail_fwhm_mac_r      = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(39))
        trail_fwhm_mac_i      = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(39))
        trail_fwhm_mac_z      = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(39))
        trail_len_pred_mac_g  = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(71))
        trail_len_pred_mac_r  = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(71))
        trail_len_pred_mac_i  = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(71))
        trail_len_pred_mac_z  = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(71))
        mcd.output_log_entry(path_logfile,'Reading in distances and magnitudes to closest sources...')
        dist_src1_mac_g       = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(44))
        dist_src1_mac_r       = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(44))
        dist_src1_mac_i       = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(44))
        dist_src1_mac_z       = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(44))
        mag_src1_mac_g        = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(45))
        mag_src1_mac_r        = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(45))
        mag_src1_mac_i        = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(45))
        mag_src1_mac_z        = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(45))
        dist_src2_mac_g       = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(50))
        dist_src2_mac_r       = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(50))
        dist_src2_mac_i       = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(50))
        dist_src2_mac_z       = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(50))
        mag_src2_mac_g        = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(51))
        mag_src2_mac_r        = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(51))
        mag_src2_mac_i        = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(51))
        mag_src2_mac_z        = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(51))
        dist_src3_mac_g       = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(56))
        dist_src3_mac_r       = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(56))
        dist_src3_mac_i       = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(56))
        dist_src3_mac_z       = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(56))
        mag_src3_mac_g        = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(57))
        mag_src3_mac_r        = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(57))
        mag_src3_mac_i        = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(57))
        mag_src3_mac_z        = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(57))
        mcd.output_log_entry(path_logfile,'Reading in global and local source densities...')
        src_density_mac_g     = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(21))
        src_density_mac_r     = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(21))
        src_density_mac_i     = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(21))
        src_density_mac_z     = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(21))
        local_srcdn_mac_g     = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(59))
        local_srcdn_mac_r     = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(59))
        local_srcdn_mac_i     = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(59))
        local_srcdn_mac_z     = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(59))
        mcd.output_log_entry(path_logfile,'Reading in distances to edges...')
        dist_edge_l_mac_g     = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(60))
        dist_edge_l_mac_r     = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(60))
        dist_edge_l_mac_i     = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(60))
        dist_edge_l_mac_z     = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(60))
        dist_edge_r_mac_g     = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(61))
        dist_edge_r_mac_r     = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(61))
        dist_edge_r_mac_i     = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(61))
        dist_edge_r_mac_z     = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(61))
        dist_edge_b_mac_g     = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(62))
        dist_edge_b_mac_r     = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(62))
        dist_edge_b_mac_i     = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(62))
        dist_edge_b_mac_z     = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(62))
        dist_edge_t_mac_g     = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(63))
        dist_edge_t_mac_r     = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(63))
        dist_edge_t_mac_i     = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(63))
        dist_edge_t_mac_z     = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(63))
        mcd.output_log_entry(path_logfile,'Reading in zero point parameters...')
        zpoint_mac_g          = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(15))
        zpoint_mac_r          = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(15))
        zpoint_mac_i          = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(15))
        zpoint_mac_z          = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(15))
        zpt_nstars_mac_g      = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(17))
        zpt_nstars_mac_r      = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(17))
        zpt_nstars_mac_i      = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(17))
        zpt_nstars_mac_z      = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(17))
        mcd.output_log_entry(path_logfile,'Reading in limiting magnitudes...')
        limit_mag_ps_mac_g    = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(18))
        limit_mag_ps_mac_r    = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(18))
        limit_mag_ps_mac_i    = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(18))
        limit_mag_ps_mac_z    = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(18))
        limit_mag_sb_mac_g    = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(19))
        limit_mag_sb_mac_r    = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(19))
        limit_mag_sb_mac_i    = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(19))
        limit_mag_sb_mac_z    = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(19))
        mcd.output_log_entry(path_logfile,'Reading in seeing measurements...')
        psf_width_mean_mac_g  = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(20))
        psf_width_mean_mac_r  = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(20))
        psf_width_mean_mac_i  = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(20))
        psf_width_mean_mac_z  = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(20))
        psf_width_src_mac_g   = np.genfromtxt(macadamia_output_g_filepath,skip_header=1,usecols=(36))
        psf_width_src_mac_r   = np.genfromtxt(macadamia_output_r_filepath,skip_header=1,usecols=(36))
        psf_width_src_mac_i   = np.genfromtxt(macadamia_output_i_filepath,skip_header=1,usecols=(36))
        psf_width_src_mac_z   = np.genfromtxt(macadamia_output_z_filepath,skip_header=1,usecols=(36))
        num_dets_mac_g        = len(detection_id_mac_g)
        num_dets_mac_r        = len(detection_id_mac_r)
        num_dets_mac_i        = len(detection_id_mac_i)
        num_dets_mac_z        = len(detection_id_mac_z)
        ...
        header_line = 'detection_id   desig_num   filt       mjd_start     mag   merr      mjd_sdss   mag_sdss  merr_sdss   mag_diff  mag_diff_err       snr   src_density   local_srcdn   dist_pred_postn   trail_predln   trail_length   trail_fwhm   dist_src1  mag_src1   dist_src2  mag_src2   dist_src3  mag_src3   dist_edge   zpoint   zpt_nstars   lim_mag_ps   lim_mag_sb   mean_seeing   src_seeing\n'
        with open(sdss_vs_macadamia_filepath_notfound,'w') as of: of.write(header_line)
        with open(sdss_vs_macadamia_filepath_all,'w') as of:      of.write(header_line)
        with open(sdss_vs_macadamia_filepath_g,'w') as of:        of.write(header_line)
        with open(sdss_vs_macadamia_filepath_r,'w') as of:        of.write(header_line)
        with open(sdss_vs_macadamia_filepath_i,'w') as of:        of.write(header_line)
        with open(sdss_vs_macadamia_filepath_z,'w') as of:        of.write(header_line)
        with open(sdss_vs_macadamia_filepath_g_or,'w') as of:     of.write(header_line)
        with open(sdss_vs_macadamia_filepath_r_or,'w') as of:     of.write(header_line)
        with open(sdss_vs_macadamia_filepath_i_or,'w') as of:     of.write(header_line)
        with open(sdss_vs_macadamia_filepath_z_or,'w') as of:     of.write(header_line)
        with open(sdss_vs_macadamia_filepath_g_hp,'w') as of:     of.write(header_line)
        with open(sdss_vs_macadamia_filepath_r_hp,'w') as of:     of.write(header_line)
        with open(sdss_vs_macadamia_filepath_i_hp,'w') as of:     of.write(header_line)
        with open(sdss_vs_macadamia_filepath_z_hp,'w') as of:     of.write(header_line)
        ...
        for idx_sdss in range(0,len(sdssmoc_table)):
            try:
                ...
                for sdss_filter in sdss_filters:
                    ...
                    if sdss_filter == 'g':
                        mac_entry_found,idx_mac,mag_diff_sdss_mac,mag_diff_err_sdss_mac = match_detection(idx_sdss,sdss_filter,mag_sdss,merr_sdss,sdssmoc_table,detection_id_mac_g,desig_num_mac_g,exp_start_jd_mac_g,mag_mac_g,magerr_mac_g,path_logfile)
                    if sdss_filter == 'r':
                        mac_entry_found,idx_mac,mag_diff_sdss_mac,mag_diff_err_sdss_mac = match_detection(idx_sdss,sdss_filter,mag_sdss,merr_sdss,sdssmoc_table,detection_id_mac_r,desig_num_mac_r,exp_start_jd_mac_r,mag_mac_r,magerr_mac_r,path_logfile)
                    if sdss_filter == 'i':
                        mac_entry_found,idx_mac,mag_diff_sdss_mac,mag_diff_err_sdss_mac = match_detection(idx_sdss,sdss_filter,mag_sdss,merr_sdss,sdssmoc_table,detection_id_mac_i,desig_num_mac_i,exp_start_jd_mac_i,mag_mac_i,magerr_mac_i,path_logfile)
                    if sdss_filter == 'z':
                        mac_entry_found,idx_mac,mag_diff_sdss_mac,mag_diff_err_sdss_mac = match_detection(idx_sdss,sdss_filter,mag_sdss,merr_sdss,sdssmoc_table,detection_id_mac_z,desig_num_mac_z,exp_start_jd_mac_z,mag_mac_z,magerr_mac_z,path_logfile)
                    
                    if mac_entry_found:
                        if sdss_filter == 'g':
                            # find distance to nearest chip edge
                            dist_edge_mac = dist_edge_l_mac_g[idx_mac]
                            if dist_edge_r_mac_g[idx_mac] < dist_edge_mac: dist_edge_mac = dist_edge_r_mac_g[idx_mac]
                            if dist_edge_t_mac_g[idx_mac] < dist_edge_mac: dist_edge_mac = dist_edge_t_mac_g[idx_mac]
                            if dist_edge_b_mac_g[idx_mac] < dist_edge_mac: dist_edge_mac = dist_edge_b_mac_g[idx_mac]
                            # write detection data to sdss vs. macadamia output file
                            output_line = '{:>12d}   {:>9d}      {:1s}   {:11.5f}   {:5.2f}  {:5.2f}   {:11.5f}      {:5.2f}      {:5.2f}      {:5.2f}         {:5.2f}   {:7.1f}   {:11.3f}   {:11.3f}   {:15.3f}   {:12.3f}   {:12.3f}   {:10.3f}   {:9.3f}  {:8.3f}   {:9.3f}  {:8.3f}   {:9.3f}  {:8.3f}   {:9.1f}   {:6.3f}   {:>10d}   {:10.3f}   {:10.3f}   {:11.3f}   {:10.3f}\n'.format(int(detection_id_mac_g[idx_mac]),int(desig_num_mac_g[idx_mac]),'g',exp_start_jd_mac_g[idx_mac],mag_mac_g[idx_mac],magerr_mac_g[idx_mac],sdssmoc_table['mjd'][idx_sdss],mag_sdss,merr_sdss,mag_diff_sdss_mac,mag_diff_err_sdss_mac,snr_mac_g[idx_mac],src_density_mac_g[idx_mac],local_srcdn_mac_g[idx_mac],dist_pred_postn_mac_g[idx_mac],trail_len_pred_mac_g[idx_mac],trail_length_mac_g[idx_mac],trail_fwhm_mac_g[idx_mac],dist_src1_mac_g[idx_mac],mag_src1_mac_g[idx_mac],dist_src2_mac_g[idx_mac],mag_src2_mac_g[idx_mac],dist_src3_mac_g[idx_mac],mag_src3_mac_g[idx_mac],dist_edge_mac,zpoint_mac_g[idx_mac],int(zpt_nstars_mac_g[idx_mac]),limit_mag_ps_mac_g[idx_mac],limit_mag_sb_mac_g[idx_mac],psf_width_mean_mac_g[idx_mac],psf_width_src_mac_g[idx_mac])
                            if snr_mac_g[idx_mac] != -999:
                                with open(sdss_vs_macadamia_filepath_all,'a') as of:
                                    of.write(output_line)
                                with open(sdss_vs_macadamia_filepath_g,'a') as of:
                                    of.write(output_line)
                                if abs(mag_mac_g[idx_mac] - mag_sdss) < outlier_threshold:
                                    with open(sdss_vs_macadamia_filepath_g_or,'a') as of:
                                        of.write(output_line)
                                else:
                                    outliers_g += 1
                                if magerr_mac_g[idx_mac] < highprecision_threshold and merr_sdss < highprecision_threshold:
                        ...
                        if sdss_filter == 'r':
                            # find distance to nearest chip edge
                            dist_edge_mac = dist_edge_l_mac_r[idx_mac]
                            if dist_edge_r_mac_r[idx_mac] < dist_edge_mac: dist_edge_mac = dist_edge_r_mac_r[idx_mac]
                            if dist_edge_t_mac_r[idx_mac] < dist_edge_mac: dist_edge_mac = dist_edge_t_mac_r[idx_mac]
                            if dist_edge_b_mac_r[idx_mac] < dist_edge_mac: dist_edge_mac = dist_edge_b_mac_r[idx_mac]
                            # write detection data to sdss vs. macadamia output file
                            output_line = '{:>12d}   {:>9d}      {:1s}   {:11.5f}   {:5.2f}  {:5.2f}   {:11.5f}      {:5.2f}      {:5.2f}      {:5.2f}         {:5.2f}   {:7.1f}   {:11.3f}   {:11.3f}   {:15.3f}   {:12.3f}   {:12.3f}   {:10.3f}   {:9.3f}  {:8.3f}   {:9.3f}  {:8.3f}   {:9.3f}  {:8.3f}   {:9.1f}   {:6.3f}   {:>10d}   {:10.3f}   {:10.3f}   {:11.3f}   {:10.3f}\n'.format(int(detection_id_mac_r[idx_mac]),int(desig_num_mac_r[idx_mac]),'r',exp_start_jd_mac_r[idx_mac],mag_mac_r[idx_mac],magerr_mac_r[idx_mac],sdssmoc_table['mjd'][idx_sdss],mag_sdss,merr_sdss,mag_diff_sdss_mac,mag_diff_err_sdss_mac,snr_mac_r[idx_mac],src_density_mac_r[idx_mac],local_srcdn_mac_r[idx_mac],dist_pred_postn_mac_r[idx_mac],trail_len_pred_mac_r[idx_mac],trail_length_mac_r[idx_mac],trail_fwhm_mac_r[idx_mac],dist_src1_mac_r[idx_mac],mag_src1_mac_r[idx_mac],dist_src2_mac_r[idx_mac],mag_src2_mac_r[idx_mac],dist_src3_mac_r[idx_mac],mag_src3_mac_r[idx_mac],dist_edge_mac,zpoint_mac_r[idx_mac],int(zpt_nstars_mac_r[idx_mac]),limit_mag_ps_mac_r[idx_mac],limit_mag_sb_mac_r[idx_mac],psf_width_mean_mac_g[idx_mac],psf_width_src_mac_r[idx_mac])
                            if snr_mac_r[idx_mac] != -999:
                                with open(sdss_vs_macadamia_filepath_all,'a') as of:
                                    of.write(output_line)
                                with open(sdss_vs_macadamia_filepath_r,'a') as of:
                                    of.write(output_line)
                                if abs(mag_mac_r[idx_mac] - mag_sdss) < outlier_threshold:
                                    with open(sdss_vs_macadamia_filepath_r_or,'a') as of:
                                        of.write(output_line)
                                else:
                                    outliers_r += 1
                                if magerr_mac_r[idx_mac] < highprecision_threshold and merr_sdss < highprecision_threshold:
                        ...
                        if sdss_filter == 'i':
                            # find distance to nearest chip edge
                            dist_edge_mac = dist_edge_l_mac_i[idx_mac]
                            if dist_edge_r_mac_i[idx_mac] < dist_edge_mac: dist_edge_mac = dist_edge_r_mac_i[idx_mac]
                            if dist_edge_t_mac_i[idx_mac] < dist_edge_mac: dist_edge_mac = dist_edge_t_mac_i[idx_mac]
                            if dist_edge_b_mac_i[idx_mac] < dist_edge_mac: dist_edge_mac = dist_edge_b_mac_i[idx_mac]
                            # write detection data to sdss vs. macadamia output file
                            output_line = '{:>12d}   {:>9d}      {:1s}   {:11.5f}   {:5.2f}  {:5.2f}   {:11.5f}      {:5.2f}      {:5.2f}      {:5.2f}         {:5.2f}   {:7.1f}   {:11.3f}   {:11.3f}   {:15.3f}   {:12.3f}   {:12.3f}   {:10.3f}   {:9.3f}  {:8.3f}   {:9.3f}  {:8.3f}   {:9.3f}  {:8.3f}   {:9.1f}   {:6.3f}   {:>10d}   {:10.3f}   {:10.3f}   {:11.3f}   {:10.3f}\n'.format(int(detection_id_mac_i[idx_mac]),int(desig_num_mac_i[idx_mac]),'i',exp_start_jd_mac_i[idx_mac],mag_mac_i[idx_mac],magerr_mac_i[idx_mac],sdssmoc_table['mjd'][idx_sdss],mag_sdss,merr_sdss,mag_diff_sdss_mac,mag_diff_err_sdss_mac,snr_mac_i[idx_mac],src_density_mac_i[idx_mac],local_srcdn_mac_i[idx_mac],dist_pred_postn_mac_i[idx_mac],trail_len_pred_mac_i[idx_mac],trail_length_mac_i[idx_mac],trail_fwhm_mac_i[idx_mac],dist_src1_mac_i[idx_mac],mag_src1_mac_i[idx_mac],dist_src2_mac_i[idx_mac],mag_src2_mac_i[idx_mac],dist_src3_mac_i[idx_mac],mag_src3_mac_i[idx_mac],dist_edge_mac,zpoint_mac_i[idx_mac],int(zpt_nstars_mac_i[idx_mac]),limit_mag_ps_mac_i[idx_mac],limit_mag_sb_mac_i[idx_mac],psf_width_mean_mac_i[idx_mac],psf_width_src_mac_i[idx_mac])
                            if snr_mac_i[idx_mac] != -999:
                                with open(sdss_vs_macadamia_filepath_all,'a') as of:
                                    of.write(output_line)
                                with open(sdss_vs_macadamia_filepath_i,'a') as of:
                                    of.write(output_line)
                                if abs(mag_mac_i[idx_mac] - mag_sdss) < outlier_threshold:
                                    with open(sdss_vs_macadamia_filepath_i_or,'a') as of:
                                        of.write(output_line)
                                else:
                                    outliers_i += 1
                                if magerr_mac_i[idx_mac] < highprecision_threshold and merr_sdss < highprecision_threshold:
                        ...
                        if sdss_filter == 'z':
                            # find distance to nearest chip edge
                            dist_edge_mac = dist_edge_l_mac_z[idx_mac]
                            if dist_edge_r_mac_z[idx_mac] < dist_edge_mac: dist_edge_mac = dist_edge_r_mac_z[idx_mac]
                            if dist_edge_t_mac_z[idx_mac] < dist_edge_mac: dist_edge_mac = dist_edge_t_mac_z[idx_mac]
                            if dist_edge_b_mac_z[idx_mac] < dist_edge_mac: dist_edge_mac = dist_edge_b_mac_z[idx_mac]
                            # write detection data to sdss vs. macadamia output file
                            output_line = '{:>12d}   {:>9d}      {:1s}   {:11.5f}   {:5.2f}  {:5.2f}   {:11.5f}      {:5.2f}      {:5.2f}      {:5.2f}         {:5.2f}   {:7.1f}   {:11.3f}   {:11.3f}   {:15.3f}   {:12.3f}   {:12.3f}   {:10.3f}   {:9.3f}  {:8.3f}   {:9.3f}  {:8.3f}   {:9.3f}  {:8.3f}   {:9.1f}   {:6.3f}   {:>10d}   {:10.3f}   {:10.3f}   {:11.3f}   {:10.3f}\n'.format(int(detection_id_mac_z[idx_mac]),int(desig_num_mac_z[idx_mac]),'z',exp_start_jd_mac_z[idx_mac],mag_mac_z[idx_mac],magerr_mac_z[idx_mac],sdssmoc_table['mjd'][idx_sdss],mag_sdss,merr_sdss,mag_diff_sdss_mac,mag_diff_err_sdss_mac,snr_mac_z[idx_mac],src_density_mac_z[idx_mac],local_srcdn_mac_z[idx_mac],dist_pred_postn_mac_z[idx_mac],trail_len_pred_mac_z[idx_mac],trail_length_mac_z[idx_mac],trail_fwhm_mac_z[idx_mac],dist_src1_mac_z[idx_mac],mag_src1_mac_z[idx_mac],dist_src2_mac_z[idx_mac],mag_src2_mac_z[idx_mac],dist_src3_mac_z[idx_mac],mag_src3_mac_z[idx_mac],dist_edge_mac,zpoint_mac_z[idx_mac],int(zpt_nstars_mac_z[idx_mac]),limit_mag_ps_mac_z[idx_mac],limit_mag_sb_mac_z[idx_mac],psf_width_mean_mac_z[idx_mac],psf_width_src_mac_z[idx_mac])
                            if snr_mac_z[idx_mac] != -999:
                                with open(sdss_vs_macadamia_filepath_all,'a') as of:
                                    of.write(output_line)
                                with open(sdss_vs_macadamia_filepath_z,'a') as of:
                                    of.write(output_line)
                                if abs(mag_mac_z[idx_mac] - mag_sdss) < outlier_threshold:
                                    with open(sdss_vs_macadamia_filepath_z_or,'a') as of:
                                        of.write(output_line)
                                else:
                                    outliers_z += 1
                                if magerr_mac_z[idx_mac] < highprecision_threshold and merr_sdss < highprecision_threshold:
                ...
                mcd.output_error_log_entry(path_logfile,path_errorfile,'Matching asteroid {:d} at {:.5f} (SDSS entry {:d}) failed.'.format(int(sdssmoc_table['ast_num'][idx_sdss]),sdssmoc_table['mjd'][idx_sdss],idx_sdss))
                mcd.output_error_log_entry_nonstring(path_logfile,path_errorfile,e)
...
def main():
    ...
    if keep_going:
        mcd.send_status_email('VL01_2_compare_sdssmoc_to_macadamia execution started','{:s} - VL01_2_compare_sdssmoc_to_macadamia execution started.'.format(datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S')))
    
        # Create and open log file
        path_logfile,path_errorfile = mcd.initialize_log_error_file(base_path,'VL01_2_compare_sdssmoc_to_macadamia')
        ...
        if not os.path.isfile(sdssmoc_filepath):
            ...
            mcd.send_status_email('VL01_2_compare_sdssmoc_to_macadamia execution failed','{:s} - VL01_compare_sdssmoc_to_macadamia execution failed - SDSS MOC file {:s} not found.'.format(datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S'),sdssmoc_filepath))
            keep_going = False

    if keep_going:
        
        macadamia_output_filepath  = base_path + 'macadamia_archive_{:s}.dat'.format(output_file_date)
        ...
        #macadamia_output_filepath  = base_path + 'macadamia_archive_20200415_s.dat.gz'
        #sdss_vs_macadamia_filepath = base_path + 'sdss_vs_macadamia_20200415_s.dat'
        #keep_going = create_sdss_macadamia_comparison_plots(sdss_vs_macadamia_filepath,keep_going,path_logfile,path_errorfile)
        #keep_going = create_sdss_macadamia_comparison_plots_outliers_removed(sdss_vs_macadamia_filepath,keep_going,path_logfile,path_errorfile)

        #macadamia_output_filepath  = base_path + 'macadamia_archive_20200415_t.dat.gz'
        #sdss_vs_macadamia_filepath = base_path + 'sdss_vs_macadamia_20200415_t.dat'
        #keep_going = create_sdss_macadamia_comparison_plots(sdss_vs_macadamia_filepath,keep_going,path_logfile,path_errorfile)
        #keep_going = create_sdss_macadamia_comparison_plots_outliers_removed(sdss_vs_macadamia_filepath,keep_going,path_logfile,path_errorfile)
        
        #macadamia_output_filepath  = base_path + 'macadamia_archive_{:s}.dat.gz'.format(output_file_date)
        #sdss_vs_macadamia_filepath = base_path + 'sdss_vs_macadamia_{:s}.dat'.format(output_file_date)
        ##keep_going = parse_sdssmoc_table(sdssmoc_filepath,sdssmoc_output_filepath,keep_going,path_logfile,path_errorfile)
        ##sdssmoc_table,keep_going = create_sdssmoc_table(sdssmoc_output_filepath,keep_going,path_logfile,path_errorfile)
        ##keep_going = create_sdss_macadamia_comparison_files(sdssmoc_table,macadamia_output_filepath,sdss_vs_macadamia_filepath,keep_going,path_logfile,path_errorfile)
        #keep_going = create_sdss_macadamia_comparison_plots(sdss_vs_macadamia_filepath,keep_going,log_file)
        ##keep_going = create_sdss_macadamia_comparison_plots_outliers_removed(sdss_vs_macadamia_filepath,keep_going,path_logfile,path_errorfile)
    ...
    mcd.send_status_email('VL01_2_compare_sdssmoc_to_macadamia execution complete.','{:s} - VL01_2_compare_sdssmoc_to_macadamia execution complete.'.format(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
