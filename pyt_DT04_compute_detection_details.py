...
def compute_detection_details(sqlite_file,output_file,thread_idx,start_detection_id,path_logfile,path_errorfile):
    try:
        ...
        query = "SELECT det.detection_id,dd.exposure_id,dd.ssobject_id,ss.desig_number FROM detections AS det JOIN detection_data AS dd JOIN ssobjects AS ss WHERE det.search_result_status='Exposure, object, and detection linked.' AND det.detection_id=dd.detection_id AND ss.ssobject_id=dd.ssobject_id"
        ...
        try:
            ...
            for _ in range(max_retries):
                try:
                    ...
                    mcd.output_log_entry(path_logfile,'JPL query failed for detection {:s}. Retrying...'.format(detection_id))
                else:
                    break
            else:
                mcd.output_error_log_entry(path_logfile,path_errorfile,'JPL query failed for detection {:s}'.format(detection_id))
                keep_going = False
        except Exception as e:
            mcd.output_error_log_entry(path_logfile,path_errorfile,'Function failed for detection {:s}: retrieve_write_detection_details_horizons()'.format(detection_id))
            print(e)
            keep_going = False
    else:
...
def main():
    ...
    if keep_going:
        mcd.send_status_email('DT04_compute_detection_details_{:02d} execution started'.format(thread_idx),'DT04_compute_detection_details_{:02d} execution started.'.format(thread_idx))
        
        # Create and initialize log file
        path_logfile,path_errorfile = mcd.initialize_log_error_file(base_path,'DT04_compute_detection_details_{:02d}'.format(thread_idx))
        ...
        output_file_details = dir_detection_data + 'detection_details_{:02d}_{:s}_toingest.txt'.format(thread_idx,datetime.datetime.today().strftime('%Y%m%d_%H%M%S'))
    ...
    mcd.send_status_email('DT04_compute_detection_details_{:02d} execution complete'.format(thread_idx),'DT04_compute_detection_details_{:02d} execution complete.'.format(thread_idx))
